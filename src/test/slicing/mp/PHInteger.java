...
public class PHInteger{ // implements DataFieldAttribute{ //[L], JifObject[L]{
...
*/
/*
    public PHInteger mult(int b){ // throws (IllegalArgumentException){
		ArrayList a = this.value;
...
//            try {
                BigIntPair pai = (BigIntPair) a.get(i);
                BigIntPair pci = new BigIntPair(pai.getX().multiply(b),pai.getY().multiply(b));
                c.add(pci);
//            } catch (NullPointerException ignored) {
//            } catch (IndexOutOfBoundsException ignored) {
//            } catch (ClassCastException ignored) {}
